...
export default function PendingRequests({ refreshTrigger, onRequestProcessed }) {
    ...
    useEffect(() => {
        // If we already have loaded once (loading is false and we might have data or empty array), try background.
        // But initially requests is empty array. relying on a separate 'initialized' ref is better but let's use a simple check
        // If loading is false, it's a background refresh (unless it's the very first render)
        // Actually, just checking if we've loaded before.
        // Let's pass true if refreshTrigger > 0
        const isBackground = refreshTrigger > 0;
